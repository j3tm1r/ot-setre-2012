...
int i;
  P1IE=0;
   delay(40,100);
   STATUS_LED_OFF;
   for(i=0;i<8;i++)PRINT_BAR(i,0);
		if ((B1)==0)
		{
			clearDisplay();
			printString("Telecomande:");
			gotoSecondLine();
		...
		if ((B4)==0)
		{
			...
			printString("Telecomande:");
			gotoSecondLine();
			printString("Button 4");
			setBargraph(8);
			while(!UTXIFG0);
			U0TXBUF = 3;
		}

	P1IFG = 0; // effacer FLAG  IRQ
